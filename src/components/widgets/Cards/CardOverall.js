import { Badge, Box, CircularProgress, Grid, Heading, Icon, Link,Text,useColorModeValue } from '@chakra-ui/react'
    ...
    <Link
      ...
      >
        ...
        <Box p={"10px"}>
          ...
          <Text fontSize={'16px'} textAlign={"right"}>{study.cliente.ci_rif}</Text>
          <Heading fontSize={'16px'}>Monto Total</Heading>
          <Text fontSize={'16px'} textAlign={"right"}>{study.total_usd}($)</Text>
        </Box>
      </Box>
    </Link >
  ));
...
export default function CardOverall_({title,content,type,toggleModal,loading}) {
  console.log(content)
  return (
    <div>
 <Heading
  ...
  >
    <Box padding={{ lg: "0px", md: "0px", sm: "0%" }}>
      ...
      :<Grid gap={"20px"} templateColumns={{ lg: "repeat(5,1fr)", md: "repeat(3,1fr)", sm: "repeat(1,1fr)" }}>
        {renderStudies(content,toggleModal)}
      </Grid>}
    </Box>
  </Box>
  </div>
