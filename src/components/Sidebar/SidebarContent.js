...
const SidebarContent = ({ routes }) => {
  ...
  const createLinks = (routes) => {
    ...
    const inactiveBg = useColorModeValue("transparent");
    const activeColor = useColorModeValue("#2B6CB0", "white");
    const inactiveColor = useColorModeValue("gray.400", "gray.400");
    const colorIcon = useColorModeValue("gray.400");
    const [user, setUsers] = useState('');
    const [group, setGroup] = useState(''); // Estado para almacenar el grupo "patologo"

    const fetchUsers = async () => {
      try {
        const tiposUsers = await usersList();
        console.log(tiposUsers);
        setUsers(tiposUsers);
        // console.log(tiposUsers);
        // Filtrar el grupo con el rol "patologo"
        const Group = tiposUsers.find((user));
        setGroup(Group);
      } catch (error) {
        console.log(error);
      }
    ...
    console.log(group);
    // const administracionRoutes = routes.filter(route => route.groupName === 'administracion');
    // const otherRoutes = routes.filter(route => route.groupName == 'patologia');
    // switch (group) {
    //   case 'administracion':
    //     return administracionRoutes.map((prop, key) => {
    //       // console.log(prop.patologia);
    //       if (prop.hide) {
    //         return null;
    //       }
    //       if (prop.category) {
    ...
    if (group === 'administracion') {
      ...
      const administracionRoutes = routes.filter(route => route.groupName === 'administracion');
      const otherRoutes = routes.filter(route => route.groupName == 'patologia');
      return otherRoutes.map((prop, key) => {
        // console.log(prop.patologia);
        if (prop.hide) {
          return null;
        }
